...
class History:
    """A pytree holding a training metric history."""
    ...
    def to_dict(self) -> dict:
        """Return the history as a JSON-serialized dictionary.
    ...
    def from_dict(cls, d: dict) -> Self:
        """Build a history from a dictionary.

        Args:
            d: A dictionary holding the data from which a new history shall
                be built. The data must be in the exact format generated by
                `History.from_dict()`.
    ...
    def save(self, path: str | Path, indent: int = 2) -> None:
        """Dump the history to a JSON file on the disk.
    ...
    def load(cls, path: str | Path) -> Self:
        """Restore a history that was saved with `History.save`.

        Args:
            path: File path to load the serialized history from.
